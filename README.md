...
QR-as-a-Service API built for AI agents. Create, update, and track dynamic QR codes programmatically via REST API or MCP (34 tools).
...
- **Custom styling** — dot shapes (square, rounded, dots, classy-rounded), corner styles, colors, gradients, logo embedding
...
- **Bulk operations** — create, update, or delete up to 50 QR codes per request
- **Multi-tenant** — each API key sees only its own data
- **MCP server** — [`qr-for-agent`](https://www.npmjs.com/package/qr-for-agent) with 34 tools for Claude Desktop, Cursor, etc.
...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/analytics/:shortId` | Scan stats with device, browser, OS, country, city breakdowns |
...
Published as [`qr-for-agent`](https://www.npmjs.com/package/qr-for-agent) on npm. 34 tools for AI agents to manage QR codes natively.
...
SQLite with [Drizzle ORM](https://orm.drizzle.team/). Five tables:
...
168 integration tests covering all endpoints, auth, multi-tenant isolation, QR types, webhooks, bulk operations, custom domains, and analytics.
